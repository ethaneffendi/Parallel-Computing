...
def traceRay(V, O = np.array([0,0,0]), d = 1):
    closest_t = None
    closest_sphere = None
...
def rayTracing_with_multiprocessing(viewportWidth, viewportHeight, imageWidth, imageHeight):
    inputList = []
    O = np.array([0,0,0])
    for y in range(imageHeight // 2, -imageHeight // 2, -1):
        for x in range(-imageWidth // 2, imageWidth // 2):
            inputList.append(canvasToViewport(x, y, viewportWidth, viewportHeight, imageWidth, imageHeight))
    pool = multiprocessing.Pool()
    outputList = pool.map(traceRay, inputList)
    pool.close()
    pool.join()
    return outputList
...
if __name__ == '__main__':
    ...
    imagePixelsList = rayTracing_with_multiprocessing(2, 2, 1000, 1000)
